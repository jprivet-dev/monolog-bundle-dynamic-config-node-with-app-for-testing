...
default_config_by_bundle: app ## Generate a YAML file with the default config for a specific bundle
	...
	$(CONSOLE) config:dump-reference $(BUNDLE) >../config/default/$(BUNDLE).yaml
...
actual_config_by_bundle: app ## Generate a YAML file with the actual config for a specific bundle, per environment (test, dev and prod).
	...
	-$(CONSOLE) debug:config $(BUNDLE) --env=test >../build/actual-app-config/$(BUNDLE).test.yaml
	-$(CONSOLE) debug:config $(BUNDLE) --env=dev >../build/actual-app-config/$(BUNDLE).dev.yaml
	-$(CONSOLE) debug:config $(BUNDLE) --env=prod >../build/actual-app-config/$(BUNDLE).prod.yaml
